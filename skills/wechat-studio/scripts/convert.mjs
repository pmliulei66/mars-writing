 ...
 *
 * 用法:
 *   node scripts/convert.mjs -i article.md [-t theme] [-o output.html] [--skip-h1]
...
const skipH1 = hasFlag(argv, ['--skip-h1'])
...
const result = renderMarkdown(markdown, theme, { skipH1 })
const { html, images, h1Title } = result
...
if (outputFile) {
  ...
  if (h1Title && !skipH1) {
    process.stderr.write(`📌 标题: ${h1Title}\n`)
  }
  if (skipH1 && h1Title) {
    process.stderr.write(`📌 H1 标题（用于草稿 JSON）: ${h1Title}\n`)
  }
  if (images.length > 0) {
